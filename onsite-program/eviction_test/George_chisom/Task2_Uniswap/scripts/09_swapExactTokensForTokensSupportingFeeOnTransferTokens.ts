...
async function main() {
  ...
  // Token addresses - Using USDC and DAI (even though they don't have transfer fees, this demonstrates the function)
  ...
  // For fee-on-transfer tokens, we can't predict exact output, so we set a conservative minimum
  const minAmountOut = ethers.parseUnits("95", 18); // Conservative minimum
  console.log("USDC to swap:", ethers.formatUnits(amountIn.toString(), 6));
  console.log("Minimum DAI expected:", ethers.formatUnits(minAmountOut.toString(), 18));
  console.log();
  ...
  const swap = await Router.connect(impersonatedSigner).swapExactTokensForTokensSupportingFeeOnTransferTokens(
    amountIn,
    minAmountOut,
    path,
  ...
  const actualRate = (daiBalAfter - daiBal) * ethers.parseUnits("1", 6) / (usdcBal - usdcBalAfter);
  console.log("Exchange Rate:", ethers.formatUnits(actualRate.toString(), 18), "DAI per USDC");
